...
public class VideoCallActivity extends CallActivity implements OnClickListener {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(cn.ucai.superwechat.R.layout.em_activity_video_call);
        ...
        RelativeLayout btnsContainer = (RelativeLayout) findViewById(cn.ucai.superwechat.R.id.ll_btns);
        ...
        netwrokStatusVeiw = (TextView) findViewById(cn.ucai.superwechat.R.id.tv_network_status);
        recordBtn = (Button) findViewById(cn.ucai.superwechat.R.id.btn_record_video);
        Button switchCameraBtn = (Button) findViewById(cn.ucai.superwechat.R.id.btn_switch_camera);
        SeekBar YDeltaSeekBar = (SeekBar) findViewById(cn.ucai.superwechat.R.id.seekbar_y_detal);
        ...
        // local surfaceview
        localSurface = (EMLocalSurfaceView) findViewById(cn.ucai.superwechat.R.id.local_surface);
        localSurface.setZOrderMediaOverlay(true);
        localSurface.setZOrderOnTop(true);

        // remote surfaceview
        oppositeSurface = (EMOppositeSurfaceView) findViewById(cn.ucai.superwechat.R.id.opposite_surface);

        // set call state listener
        addCallStateListener();
        if (!isInComingCall) {// outgoing call
            soundPool = new SoundPool(1, AudioManager.STREAM_RING, 0);
            outgoing = soundPool.load(this, cn.ucai.superwechat.R.raw.em_outgoing, 1);
                ...
                case ACCEPTED: // call is accepted
                    ...
                    runOnUiThread(new Runnable() {
                        ...
                        public void run() {
                            ...
                            openSpeakerOn();
                            ((TextView)findViewById(cn.ucai.superwechat.R.id.tv_is_p2p)).setText(EMClient.getInstance().callManager().isDirectCall()
                                    ? cn.ucai.superwechat.R.string.direct_call : cn.ucai.superwechat.R.string.relay_call);
                            handsFreeImage.setImageResource(cn.ucai.superwechat.R.drawable.em_icon_speaker_on);
                    ...
                    runOnUiThread(new Runnable() {
                        public void run() {
                            ...
                            if(error == CallError.ERROR_NO_DATA){
                                netwrokStatusVeiw.setText(cn.ucai.superwechat.R.string.no_call_data);
                            }else{
                                netwrokStatusVeiw.setText(cn.ucai.superwechat.R.string.network_unstable);
                            }
                        }
                    });
                    break;
                    ...
                    runOnUiThread(new Runnable() {
                        ...
                        public void run() {
                            ...
                            if (fError == CallError.REJECTED) {
                                ...
                                callStateTextView.setText(cn.ucai.superwechat.R.string.call_version_inconsistent);
                            }  else {
                                if (isAnswered) {
                                    callingState = CallingState.NORMAL;
...
//                                        callStateTextView.setText(s6);
    ...
    public void onClick(View v) {
        switch (v.getId()) {
        case cn.ucai.superwechat.R.id.btn_refuse_call: // decline the call
            refuseBtn.setEnabled(false);
            handler.sendEmptyMessage(MSG_CALL_REJECT);
            break;

        case cn.ucai.superwechat.R.id.btn_answer_call: // answer the call
        ...
        case cn.ucai.superwechat.R.id.btn_hangup_call: // hangup
        ...
        case cn.ucai.superwechat.R.id.iv_mute: // mute
            if (isMuteState) {
                // resume voice transfer
                muteImage.setImageResource(cn.ucai.superwechat.R.drawable.em_icon_mute_normal);
                EMClient.getInstance().callManager().resumeVoiceTransfer();
                isMuteState = false;
            } else {
                // pause voice transfer
                muteImage.setImageResource(cn.ucai.superwechat.R.drawable.em_icon_mute_on);
                EMClient.getInstance().callManager().pauseVoiceTransfer();
                isMuteState = true;
            }
            break;
        case cn.ucai.superwechat.R.id.iv_handsfree: // handsfree
            if (isHandsfreeState) {
                // turn off speaker
                handsFreeImage.setImageResource(cn.ucai.superwechat.R.drawable.em_icon_speaker_normal);
                closeSpeakerOn();
            ...
            break;
        case cn.ucai.superwechat.R.id.btn_record_video: //record the video
            if(!isRecording){
                callHelper.startVideoRecord(PathUtil.getInstance().getVideoPath().getAbsolutePath());
                isRecording = true;
                recordBtn.setText(cn.ucai.superwechat.R.string.stop_record);
            }else{
                String filepath = callHelper.stopVideoRecord();
                isRecording = false;
                recordBtn.setText(cn.ucai.superwechat.R.string.recording_video);
                Toast.makeText(getApplicationContext(), String.format(getString(cn.ucai.superwechat.R.string.record_finish_toast), filepath), Toast.LENGTH_LONG).show();
            }
            break;
        case cn.ucai.superwechat.R.id.root_layout:
            ...
            break;
        case cn.ucai.superwechat.R.id.btn_switch_camera: //switch camera
